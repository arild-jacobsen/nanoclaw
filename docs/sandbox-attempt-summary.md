...
## Source commits (all on `main` / `origin/main`, authored by Arild)
...
A fourth commit — the one that adds this summary file — also commits two
previously-uncommitted bug fixes to `src/container-runner.ts` and `start.sh`.

**Branch layout:** the full sandbox attempt (all of the above) is preserved on
the **`docker-sandbox`** branch. `main` is being cleaned of sandbox-specific
changes per the "Summary for cleanup" section below. This file itself stays on
`main` as the record of what was removed and where it now lives.

The original analysis doc `docs/sandbox-adjustments.md` (created in `52798c1`) is
the empirical writeup of how the sandbox environment behaves; this file is the
change ledger.
